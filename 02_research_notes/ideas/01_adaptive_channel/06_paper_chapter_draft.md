...
5. **ILI 负结果的诚实呈现（P0-1 后补充, 2026-08-18）**:
   ILI（7 通道, 最小样本）在 pl24 上 **-11.51% 显著变差**（8-seed 配对）。
   这不是"低维不占优"的模糊表述, 而是明确的负向显著: 在低维 + 小样本 regime,
   稳定性门控引入的约束 > 信息量, 主动损害预测。适用边界须表述为
   **"高维（通道数 ≥ 21）∧ 短 horizon（≤96）"双条件**;
   低维或长 horizon 皆不适用（etth1 pl96 -1.80%*、ettm1 pl336 -2.18%*、weather pl336 -1.35%
   同样佐证）。ILI pl24 是论文中最诚实的锚点: 说明该方法不是"通用增益",
   而是有明确适用区间的正则化手段。
6. **修复版带宽的边界（P0-1 后补充）**: 修 A+B（median 带宽 + CKA 归一化）在
   weather (21ch, -3.25%* 8/8 seed) 与 electricity (321ch, -2.01%*/8/8) 全面有效,
   但在 traffic (862ch) 失效（+1.18%, 0/8 seed 一致变差）—— median heuristic 带宽
   在超高维失真。论文表述: 修复版适用区间为中等通道数; 超高维下仍推荐旧版（容量红利主导）。

7. **syn_ood 机制测试 (2026-08-23, ⭐ 重大正面发现)**: 修复版 full_v2_fixed 在合成
   OOD 数据上 **+44.2~44.8% 显著超越 patchtst (8 配置一致, 0 调参必要)**;
   旧 full_v2 仅 +0.66% 打平。**机制测试从 "未通过" 升级为 "直接证据"**。这意味
   着 OOD 鲁棒性宣称从 "受限" 升级为 "有强证据"。

8. **修 C 语义切分止损 (2026-08-23)**: uniform vs semantic (season) 5/5 组均
   semantic 略差 (+0.06~+2.64%); 批量内每环境 ~8 样本, HSIC 估计方差大; 数据上有
   信息 ≠ 训练时门控能用上。与方案 1 "语义信息量 5/17 无预测力" 一致。
   **不报告语义切分方案, 改报告 "负结果 + 原因分析" 作为诚实边界**。

9. **DRO 弱正信号 (2026-08-23)**: λ=0.1 全部 5 组不差于 ERM, weather pl192 单调
   改善 1.16%; 提升 < 1% 远不及 full_v2_fixed vs patchtst 的 +3~9%。不作为主要
   贡献, 配对显著性待补。

10. **高维门控诊断 (2026-08-23, ⭐)**: full_v2_fixed (off_std=0.062, collapse=0%,
    batch_dep≈0) 达到最佳平衡; 旧 full_v2 batch_dep=0.34 跨 batch 不稳定被修复 A+B
    解决。electricity 热图 (50/321 通道子采样) 呈现清晰块对角结构, 与电力客户聚类
    一致 → **门控选到了真实依赖结构**。
