...
class CARDformer(nn.Module):
    def __init__(self, 
        ...
        self.dname = config.data_path.split('.csv')[0]
        ...
        self.alpha_ds = nn.Parameter(torch.Tensor([1.0])*config.init_alpha)#.to(config.gpu)
        self.beta_ds = nn.Parameter(torch.Tensor([0.0]))#.to(config.gpu)
    ...
    def forward(self, z,*args, **kwargs):     
        ...
        for a_2,a_1,mlp,drop,norm  in zip(self.Attentions_over_token, self.Attentions_over_channel,self.Attentions_mlp ,self.Attentions_dropout,self.Attentions_norm ):
            ...
            output_1 = a_1(inputs.permute(0,2,1,3), cm.to(inputs.device)).permute(0,2,1,3)
...
class Attenion(nn.Module):
    ...
    def forward(self, src, cm=None, *args,**kwargs):
        ...
        if not self.over_hidden: 
            ...
            attn_score_along_token = torch.einsum('bnhed,bnhfd->bnhef', self.ema(q), self.ema(k))/ self.head_dim ** -0.5
            attn_score_along_token = attn_score_along_token * cm.unsqueeze(0).unsqueeze(0).unsqueeze(0)
            #print(attn_score_along_token.shape,cm.shape)
            #print('FIX attn_score_along_token',attn_score_along_token.shape)
            attn_along_token = self.attn_dropout(F.softmax(attn_score_along_token, dim=-1) )
            output_along_token = torch.einsum('bnhef,bnhfd->bnhed', attn_along_token, v)
