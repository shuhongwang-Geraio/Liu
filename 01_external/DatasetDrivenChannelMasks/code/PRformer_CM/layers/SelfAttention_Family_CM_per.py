...
class FullAttention(nn.Module):
    def __init__(self, init_alpha, mask_flag=True, factor=5, scale=None, attention_dropout=0.1, output_attention=False):
        super(FullAttention, self).__init__()
        self.scale = scale
        self.mask_flag = mask_flag
        self.output_attention = output_attention
        self.dropout = nn.Dropout(attention_dropout)
        self.alpha_ds = nn.Parameter(torch.Tensor([1.0])*init_alpha)#.to(configs.gpu)
        self.beta_ds = nn.Parameter(torch.Tensor([0.0]))#.to(configs.gpu)

    def forward(self, queries, keys, values, corr, attn_mask, tau=None, delta=None):
        cm = transform_corr(corr, self.alpha_ds, self.beta_ds)
        B, L, H, E = queries.shape
        _, S, _, D = values.shape
        scale = self.scale or 1. / sqrt(E)
        ...
        pad_C = scores.shape[-1] - cm.shape[-1]
        cm = F.pad(cm, (0, pad_C, 0, pad_C), mode='constant', value=1)
        A = self.dropout(torch.softmax(scale * scores * cm.unsqueeze(0).unsqueeze(0), dim=-1))
        V = torch.einsum("bhls,bshd->blhd", A, values)
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, cm, attn_mask, tau=None, delta=None):
        B, L, _ = queries.shape
        _, S, _ = keys.shape
        H = self.n_heads
