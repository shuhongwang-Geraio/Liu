...
class FullAttention(nn.Module):
    ...
    def forward(self, values, attn_mask, tau=None, delta=None):
        B, L, H, E = values.shape
        _, S, _, D = values.shape
        scale = self.scale or 1. / sqrt(E)
        attn_mask = F.pad(attn_mask, 
                              (0, L-attn_mask.shape[0], 0, L-attn_mask.shape[0]), 
                              mode='constant', value=1)
        attn_mask = attn_mask.unsqueeze(0).unsqueeze(0).repeat(B,H,1,1)
        A = self.dropout(torch.softmax(scale  * attn_mask, dim=-1))
