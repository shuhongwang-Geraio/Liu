...
class VarAttention(nn.Module):
    ...
    #================================================================================#
    def att_with_CM(self, query, key, value, 
                                      attn_mask=None, dropout_p=0.0, scale=None) -> torch.Tensor:
...
class VarAttBlock(nn.Module):
    ...
    def forward(self, x, att_mask):
        x = x + self.drop_path1(self.ls1(self.attn_var(self.norm1(x),att_mask)))
        return x
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, prefix_seq_len, attn_mask, CM):
        x = self.seq_att_block(x, attn_mask)
        x = self.var_att_block(x, CM)
...
class Model(nn.Module):
    ...
    def __init__(self, args, configs_list, pretrain=False):
        ...
        self.channel_mask = dict()
        self.domain_alpha = nn.ParameterDict({})
        self.domain_beta = nn.ParameterDict({})
        

        for i in range(self.num_task):
            dataset_name = configs_list[i][1]['dataset']
            self.domain_alpha[dataset_name] = nn.Parameter(torch.Tensor([2.0]))
            self.domain_beta[dataset_name] = nn.Parameter(torch.Tensor([0.0]))
            att_m = np.abs(np.load(f'corr/{dataset_name}.npy'))
            self.channel_mask[dataset_name] = att_m
            if C==1:
                self.channel_mask[dataset_name] = None
    ...
    def backbone(self, x, prefix_len, seq_len, CM):
        attn_mask = None
        for block in self.blocks:
            x = block(x, prefix_seq_len=prefix_len +
                      seq_len, attn_mask=attn_mask, CM=CM)
        return x
    ...
    def forecast(self, x, x_mark, task_id):
        ...
        CM = self.channel_mask[dataset_name]
        
        if CM is not None:
            CM = transform_corr(CM, self.domain_alpha[dataset_name], self.domain_beta[dataset_name])
        x = self.backbone(x, prefix_prompt.shape[2], seq_token_len, CM)
    ...
    def classification(self, x, x_mark, task_id):
        ...
        if CM is not None:
            CM = transform_corr(CM, self.domain_alpha[dataset_name], self.domain_beta[dataset_name])
        x = self.backbone(x, prefix_prompt.shape[2], seq_len, CM)
