...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
                 max_seq_len: Optional[int] = 1024, n_layers: int = 3, d_model=128,
                 n_heads=16, d_k: Optional[int] = None, d_v: Optional[int] = None,
...
class _ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, prev: Optional[Tensor] = None,
        ...
        if attn_mask is not None:                               # attn_mask with shape [q_len x seq_len]
        ...
        if key_padding_mask is not None:                        # mask with shape [bs x q_len]
